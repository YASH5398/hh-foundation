...
export async function createAgentNotification({ type, title, message, userId, userName, priority = AGENT_NOTIF_PRIORITIES.MEDIUM }) {
  try {
    ...
    console.error(String('Error creating agent notification:') + " " + String(error));
...
export function listenToAgentNotifications(callback, onError) {
  ...
  const unsubscribe = onSnapshot(
    ...
    (error) => {
      ...
      if (!hasLoggedError) {
        console.error(String('[AgentNotifications] Listener error:') + " " + String(error.code || error.message));
        hasLoggedError = true;
      }
...
export async function markAgentNotificationAsRead(notificationId) {
  try {
    ...
    console.error(String('Error marking agent notification as read:') + " " + String(error));
    return { success: false, error };
  }
}
...
export async function markAllAgentNotificationsRead() {
  try {
    ...
    console.error(String('Error [markAllAgentNotificationsRead]:') + " " + String(error));
    return { success: false, error };
  }
}