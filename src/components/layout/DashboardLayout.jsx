...
const DashboardLayout = () => {
  ...
  const { user, userProfile, logout, testLogout } = useAuth();
  ...
  useEffect(() => {
    ...
    const handleOpenSidebar = () => {
      console.log('DashboardLayout: Received open-dashboard-sidebar event');
      setIsSidebarOpen(true);
    };

    window.addEventListener('open-dashboard-sidebar', handleOpenSidebar);

    return () => {
      window.removeEventListener('resize', checkMobile);
      window.removeEventListener('open-dashboard-sidebar', handleOpenSidebar);
    };
  }, []);
  ...
  const handleLogout = async () => {
    ...
    try {
      console.log("🔍 DASHBOARD LAYOUT: Calling AuthContext logout...");
      const result = await logout();
      console.log(String("🔍 DASHBOARD LAYOUT: AuthContext logout result:") + " " + String(result));

      if (result.success) {
        console.log("🔍 DASHBOARD LAYOUT: Logout successful, navigating to /login...");
        navigate('/login', { replace: true });
        console.log("🔍 DASHBOARD LAYOUT: Navigation to /login completed");
      } else {
        console.error(String("🔍 DASHBOARD LAYOUT: Logout failed:") + " " + String(result.error));
      }
    } catch (error) {
      console.error(String("🔍 DASHBOARD LAYOUT: Exception during logout:") + " " + String(error));
    }
  };
