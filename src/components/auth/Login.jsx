...
const Login = () => {
  ...
  useEffect(() => {
    console.log(String("🔍 LOGIN: Location changed to:") + " " + String(location.pathname));
  }, [location]);

  // Redirect authenticated users away from login
  React.useEffect(() => {
    console.log(String("🔍 LOGIN COMPONENT:") + " " + String({
      user: !!user,
    ...
    // Only redirect if:
    // 1. User is authenticated in React state
    // 2. Firebase auth confirms the user is logged in
    // 3. We're on the login page
    // 4. Not in a loading state
    if (user && auth.currentUser && !authLoading && location.pathname === '/login' && user.uid) {
      console.log("🔍 LOGIN: User authenticated (both React + Firebase), redirecting to dashboard");
      navigate("/dashboard", { replace: true });
    } else if (!user && !auth.currentUser && location.pathname !== '/login') {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(String("🔍 LOGIN: Auth context login result:") + " " + String(result));
      ...
      if (result.success) {
        ...
        console.log(String("🔍 LOGIN: Auth failed:") + " " + String(result));
        // Error toast is already handled in the context's login function
      }
    } catch (error) {
      console.error(String("🔍 LOGIN: Exception during login:") + " " + String(error));
