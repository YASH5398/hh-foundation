...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      ...
      try {
        ...
        console.log(String('currentUser.uid:') + " " + String(user.uid));
        console.log(String('currentUser.userId:') + " " + String(user.userId));
        // Total Sent Help
        ...
        unsubSentHelp = onSnapshot(sentHelpQ, (snap) => {
          ...
          console.log(String('sendHelp docs:') + " " + String(snap.docs.slice(0, 3).map((d) => d.data())));
        }, (error) => {
        ...
        unsubAutoPool = onSnapshot(autoPoolQ, (snap) => {
          ...
          console.log(String('receiveHelp (AutoPool) docs:') + " " + String(snap.docs.slice(0, 3).map((d) => d.data())));
        }, (error) => {
          // Immediately unsubscribe on permission-denied
          if (error.code === 'permission-denied') {
            unsubAutoPool();
        ...
        unsubSponsor = onSnapshot(sponsorQ, (snap) => {
          ...
          console.log(String('receiveHelp (Sponsor) docs:') + " " + String(snap.docs.slice(0, 3).map((d) => d.data())));
        }, (error) => {
          // Immediately unsubscribe on permission-denied
          if (error.code === 'permission-denied') {
            unsubSponsor();
        ...
        unsubDirect = onSnapshot(directQ, (snap) => {
          ...
          console.log(String('users (Direct Members) docs:') + " " + String(snap.docs.slice(0, 3).map((d) => d.data())));
        });
        // Available E-PINs
        ...
        console.error(String('Failed to setup dashboard listeners:') + " " + String(error));
      }
    };
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (userDoc.exists()) {
          ...
          console.log(String("UID:") + " " + String(user.uid));
          console.log(String('📁 Firestore path:') + " " + String(`users/${user.uid}`));
  ...
  const fetchRecentActivity = async () => {
    ...
    try {
      ...
      console.error(String('Error fetching recent activity:') + " " + String(error));
    } finally {
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      ...
      try {
            ...
            const receivers = await Promise.all(
              docs.map(async (doc) => {
                ...
                try {
                  ...
                  console.error(String('Error fetching receiver info:') + " " + String(error));
                }
        ...
        console.error(String('Failed to setup upcoming payments listener:') + " " + String(error));
        setUpcomingLoading(false);
      }
    };
      ...
      try {
        ...
        unsubscribe = onSnapshot(sendHelpQuery, async (snapshot) => {
          try {
            if (!snapshot.empty) {
              ...
              if (latestSendHelp.receiverUid) {
                try {
                  ...
                  console.error(String('Error fetching receiver details:') + " " + String(error));
                }
              }
            } else {
            ...
            console.error(String('Error fetching send help reminder:') + " " + String(error));
          } finally {
            setReminderLoading(false);
          }
        });
      } catch (error) {
        console.error(String('Failed to setup sendHelp reminder listener:') + " " + String(error));
        setReminderLoading(false);
      }
    };
  ...
  const fetchLeaderboard = async (selectedLevel) => {
    ...
    try {
      ...
      console.error(String("Error fetching leaderboard:") + " " + String(error));
