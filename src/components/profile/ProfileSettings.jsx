...
export default function ProfileSettings() {
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      ...
      console.error(String('Error changing password:') + " " + String(error));
  ...
  const handleResetPassword = async () => {
    try {
      ...
      console.error(String('Error sending password reset email:') + " " + String(error));
      ...
      if (user?.uid) {
        try {
          ...
          if (userDoc.exists()) {
            ...
            if (paymentMethods.upi) setSelectedPaymentMethod('upi');else
            if (paymentMethods.gpay) setSelectedPaymentMethod('gpay');else
            if (paymentMethods.phonePe) setSelectedPaymentMethod('phonePe');else
            if (paymentMethods.bank) setSelectedPaymentMethod('bank');
          }
        } catch (error) {
  ...
  const handleReset = () => {
    ...
    if (paymentMethods.gpay) setSelectedPaymentMethod('gpay');else
    if (paymentMethods.phonePe) setSelectedPaymentMethod('phonePe');else
    if (paymentMethods.bank) setSelectedPaymentMethod('bank');else
  ...
  const handleUploadAvatar = async (e) => {
    try {
      ...
      console.error(String('Error uploading avatar:') + " " + String(error));
      ...
      try {
        ...
        console.error(String('Error updating Firestore with default image:') + " " + String(firestoreError));
      }
    } finally {
      setUploading(false);
      setUploadProgress(0);
