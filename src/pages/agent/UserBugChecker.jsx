...
import {
  ...
  FiExternalLink, FiFileText, FiImage, FiPaperclip } from
'react-icons/fi';
import { motion, AnimatePresence } from 'framer-motion';
import {
  collection, query, where, orderBy, limit, startAfter,
  getDocs, doc, getDoc, onSnapshot, updateDoc } from
'firebase/firestore';
...
const UserBugChecker = () => {
  ...
  const loadBugTickets = async (loadMore = false) => {
    try {
      ...
      for (const docSnap of snapshot.docs) {
        ...
        if (ticketData.userId) {
          try {
            ...
            console.error(String('Error loading user info:') + " " + String(error));
          }
        }
      ...
      console.error(String('Error loading bug tickets:') + " " + String(error));
  ...
  const loadTicketMessages = async (ticketId) => {
    try {
      ...
      console.error(String('Error loading ticket messages:') + " " + String(error));
  ...
  const exportBugTickets = () => {
    try {
      ...
      console.error(String('Error exporting bug tickets:') + " " + String(error));
