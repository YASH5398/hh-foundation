...
import {
  ...
  FiDollarSign, FiAlertTriangle, FiFlag } from
'react-icons/fi';
...
const UserManagement = () => {
  ...
  const loadUserStats = async (uid) => {
    try {
      ...
      ticketsSnapshot.docs.forEach((doc) => {
        ...
        if (ticket.status === 'resolved' || ticket.status === 'closed') stats.resolvedTickets++;else
      ...
      console.error(String('Stats Error:') + " " + String(error));
    }
  };
  ...
  const searchUser = async (e) => {
    ...
    try {
      ...
      console.error(String('Search error:') + " " + String(error));
  ...
  const exportUsers = () => {
    try {
      ...
      console.error(String('Error exporting users:') + " " + String(error));
