...
import {
  ...
  FiFlag, FiBox, FiCheck, FiChevronRight, FiMaximize2, FiArrowLeft } from
'react-icons/fi';
import { motion, AnimatePresence } from 'framer-motion';
import {
  collection, query, where, onSnapshot, orderBy,
  doc, updateDoc, addDoc, serverTimestamp, getDoc, limit } from
'firebase/firestore';
...
const SupportTickets = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(ticketsQuery, (snapshot) => {
      ...
      console.error(String('Error fetching tickets:') + " " + String(error));
    ...
    try {
      ...
      if (selectedTicket.userId) {
        try {
          ...
          console.error(String("Failed to send notification") + " " + String(noteErr));
