...
export async function cleanSendHelpDuplicates() {
  const snapshot = await getDocs(collection(db, 'sendHelp'));
  ...
  for (const key in groups) {
    ...
    if (docs.length > 3) {
      ...
      for (const docData of toDelete) {
        await deleteDoc(doc(db, 'sendHelp', docData.id));
        deletedCount++;
        console.log('✅ Deleted excess sendHelp:', docData.id, 'for receiver/level:', key);
      }
    }
  }
  console.log(`✅ Duplicate cleanup done. Deleted ${deletedCount} excess sendHelp doc(s).`);
  return deletedCount;
} 